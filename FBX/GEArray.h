...
public:
	...
	bool InitArray(__int32 nElementSize, __int32 nInitCapacity, __int32 nAddCount);
	//申请更大的内存，存储更多的元素。
	//如果newCapacity比当前的m_nCapacity小，则什么都不做。
	bool ResizeArray(__int32 newCapacity);
	//清空容器。
	void ClearArray();
	//把新元素添加到队尾。
	//返回值为新元素的位置索引。
	__int32 PushBack(const void* pElement);
	//申请一个新元素，由外界来填充新元素的值。
	//返回值为新元素的位置索引。
	__int32 TakeNew(void** ppElement);
	//把新元素插入到nIndex位置，原nIndex位置的元素以及nIndex位置后面的元素，都顺次后移一个位置。
	//如果nIndex为无效值的话，则会返回修正之后的位置索引。
	//返回值为新元素的位置索引。
	__int32 InsertAt(__int32 nIndex, const void* pElement);
	//移除某个元素。
	bool RemoveAt(__int32 nIndex);
	bool Remove(const void* pElement);
	//两个元素交换位置。
	bool SwapAt(__int32 nIndexA, __int32 nIndexB);
	//获取某个元素。
	void* GetAt(__int32 nIndex) const;
	//获取有效元素的个数。
	__int32 GetSize() const;
	//获取容器的大小。
	__int32 GetCapacity() const;
	//获取某个元素的位置索引。
	...
	__int32 GetIndex(const void* pElement, __int32 nValidSize) const;

private:
	//在m_nDeltaAddRemove优化策略下，本函数主要用来寻找空洞。
	__int32 FindEmptyElement() const;

private:
	typedef __int32 arrayuid;
...
private:
	...
	__int32 m_nDeltaAddRemove;
};
//----------------------------------------------------------------
inline void GEArray::ClearArray()
{
	m_nSize = 0;
	m_nDeltaAddRemove = 0;
}
//----------------------------------------------------------------
inline __int32 GEArray::GetSize() const
{
	return m_nSize;
}
//----------------------------------------------------------------
inline __int32 GEArray::GetCapacity() const
{
	return m_nCapacity;
}
//----------------------------------------------------------------
