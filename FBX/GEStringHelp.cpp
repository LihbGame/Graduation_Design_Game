...
//1，致力于跨平台，跨编译器。
//2，非线程安全，使用了全局的字符串缓存。
//   GExxxSelf 系列的函数是线程安全函数，它要求用户提供字符串缓存。
//--------------------------------------------------------------------
//去除编译warning
...
#include <stdlib.h>
//--------------------------------------------------------------------
#define GEStringHelp_MaxStringSize 0x3FFFFFFF
//本模块使用的缓存
char g_StrHelpBuff[GEStringHelp_GlobalBuffSize] = {0};
wchar_t* g_WStrHelpBuff = (wchar_t*)g_StrHelpBuff;
//--------------------------------------------------------------------
int GEStrLen(const char* szString)
{
	int nLen = 0;
	for (nLen = 0; nLen < GEStringHelp_MaxStringSize; ++nLen)
	{
		if (*szString == 0)
		{
			break;
...
char* GEStrFmt(const char* szFormat, ...)
{
	...
	vsnprintf(g_StrHelpBuff, GEStringHelp_GlobalBuffSize, szFormat, klist);
	va_end(klist);
...
char* GEStrFmtByVaList(const char* szFormat, const void* pValist)
{
	...
	vsnprintf(g_StrHelpBuff, GEStringHelp_GlobalBuffSize, szFormat, *pList);
	return g_StrHelpBuff;
}
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
int GEStrChr(const char* str, const char c, int nStrLen)
{
	int nPos = -1;
	if (nStrLen == -1)
	{
		nStrLen = GEStrLen(str);
	}
	//
...
//--------------------------------------------------------------------
int GEWStrChr(const wchar_t* str, const wchar_t c, int nStrLen)
{
	int nPos = -1;
	if (nStrLen == -1)
	{
		nStrLen = GEWStrLen(str);
	}
	//
	for (int i = 0; i < nStrLen; ++i)
	{
...
//--------------------------------------------------------------------
int GEStrRChr(const char* str, const char c, int nStrLen)
{
	int nPos = -1;
	if (nStrLen == -1)
	{
...
//--------------------------------------------------------------------
int GEWStrRChr(const wchar_t* str, const wchar_t c, int nStrLen)
{
	int nPos = -1;
	if (nStrLen == -1)
	{
...
{
	int nSubStrLen = 0;
	for (int i = 0; i < GEStringHelp_MaxStringSize; ++i)
	{
		if (str[i] == 0)
		{
			break;
		...
		else if (i >= nStartIndex && i <= nEndIndex)
		{
			...
			++nSubStrLen;
			if (nSubStrLen >= GEStringHelp_GlobalBuffSize - 1)
			{
				break;
			}
		}
...
//--------------------------------------------------------------------
char* GEStrSlim(const char* str, int nStrLen)
{
	g_StrHelpBuff[0] = 0;
	if (nStrLen == -1)
	{
	...
	if (nValidCount > GEStringHelp_GlobalBuffSize - 1)
	{
		nValidCount = GEStringHelp_GlobalBuffSize - 1;
	}
...
char* GEStrToUpper(const char* szString)
{
	...
	while (szString[nCount] != 0)
	{
		...
		//
		++nCount;
		if (nCount >= GEStringHelp_GlobalBuffSize)
		{
			g_StrHelpBuff[GEStringHelp_GlobalBuffSize-1] = 0;
			break;
		}
	}
	if (nCount < GEStringHelp_GlobalBuffSize)
...
char* GEPathNormalize(const char* szPath, bool bXieGang)
{
	...
	if (nCount < GEStringHelp_GlobalBuffSize)
	{
		...
		if (bXieGang)
		{
			...
			{
				if (nCount+1 < GEStringHelp_GlobalBuffSize)
				{
...
double GEatof(const char* szString)
{
	...
	const int nStrLen = GEStrCpy(g_StrHelpBuff, GEStringHelp_GlobalBuffSize, szString);
	// strip string, only remain 7 numbers after '.'
	int nPos = GEStrChr(g_StrHelpBuff, '.', nStrLen);
	if (nPos != -1 && nPos + 8 < GEStringHelp_GlobalBuffSize)
	{
...
double GEWatof(const wchar_t* szString)
{
	...
	const int nStrLen = GEWStrCpy(g_WStrHelpBuff, GEStringHelp_GlobalBuffSize, szString);
	// strip string, only remain 7 numbers after '.'
	int nPos = GEWStrChr(g_WStrHelpBuff, '.', nStrLen);
	if (nPos != -1 && nPos + 8 < GEStringHelp_GlobalBuffSize)
	{
