...
MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	return D3DApp::Get()->MsgProc(hwnd, msg, wParam, lParam);
}
...
bool D3DApp::Init()
{
	...
	//gui init
	m_pGameGUI->Init(md3dImmediateContext, mClientWidth, mClientHeight);
	g_XamlView = m_pGameGUI->GetXamlView();
	g_KeyMap = m_pGameGUI->GetKeyMap();
...
LRESULT D3DApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		if (md3dDevice)
		{
			...
			else if (wParam == SIZE_RESTORED)
			{
				...
				{
					// If user is dragging the resize bars, we do not resize H
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (g_XamlView != 0)
		{
			g_XamlView->MouseButtonDown(LOWORD(lParam), HIWORD(lParam), Noesis::MouseButton::MouseButton_Left);
	...
	case WM_LBUTTONUP:
	{
		if (g_XamlView != 0)
		{
			...
			g_XamlView->MouseButtonUp(LOWORD(lParam), HIWORD(lParam), Noesis::MouseButton::MouseButton_Left);
	...
	case WM_KEYUP:
	{
		...
		case 'M':
		{
			...
			if (!menu)
			{
				ShowWindow(hWnd32, SW_SHOW);
				ShowWindow(hwnd, SW_HIDE);
				//launcher.GetDisplay()->SetTopmost(true);
				menu = true;
			}
			else
			{
...
bool D3DApp::InitMainWindow()
{
	...
	mhMainWnd = CreateWindow(LPCSTR("D3DWndClassName"), LPCSTR(mMainWndCaption.c_str()),
		WS_POPUP, 0, 0, mClientWidth, mClientHeight, hWnd32, 0, mhAppInst, 0);
...
void D3DApp::CalculateFrameStats()
{
	...
	if( (mTimer.TotalTime() - timeElapsed) >= 1.0f )
	{
		...
		SetWindowText(mhMainWnd, LPCSTR(outs.str().c_str()));
