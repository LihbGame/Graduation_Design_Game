...
private:
    ...
    public:
        ...
        int x, y; // 坐标
        int s; // 步数
        std::shared_ptr<CNode> parent; // 父结点
    };
    ...
    bool bfs_item(int x, int y)
   {
            ...
            // 遍历周边的点,放入队列
            {
                ...
                //左
                {
                    ...
                    if(!(nx < 0 || nx >= m_row || ny < 0 || ny >= m_col) && m_mark[nx][ny] == false) {
                        if( m_arr[curr.x][curr.y].m_bLeft) {
                ...
                //右
                {
                    ...
                    int ny = curr.y + 1;
                    if(!(nx < 0 || nx >= m_row || ny < 0 || ny >= m_col) && m_mark[nx][ny] == false) {
                        if(m_arr[curr.x][curr.y].m_bRight) {
                            q_open.push(CNode(nx, ny, 1 + curr.s, new CNode(curr)));
                            m_mark[nx][ny] = true;
                        }
                    }
                ...
                //下
                {
                    ...
                    int nx = curr.x + 1;
                    int ny = curr.y;
                    if( m_arr[curr.x][curr.y].m_bBottom) {
                        if(!(nx < 0 || nx >= m_row || ny < 0 || ny >= m_col) && m_mark[nx][ny] == false) {
                            q_open.push(CNode(nx, ny, 1 + curr.s, new CNode(curr)));
                            m_mark[nx][ny] = true;
                        }
    ...
    bool bfs(int x, int y, int m_targetX, int m_targetY, VPath &path)
    {
        ...
        for(int i = 0; i < m_row; ++i) {
            std::vector<bool> vLine(m_col, false);
        ...
        if(bFind) {
            std::copy(m_path.rbegin(), m_path.rend(), std::back_inserter<VPath>(path));
        }

        return bFind;
    };
    std::vector<std::vector<CNode>> m_nodes;
};
